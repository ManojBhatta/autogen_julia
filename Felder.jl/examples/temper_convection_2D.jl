...
no motion. Assuming 2D plane strain.

A predefined paraview state file (.pvsm) is provided in
"output/temper_convection_2D_paraviewstate.pvsm"
=#
...
h_edge(x, t)        = t < 10 ? 450.0 : 20.0 # Convection coefficient [W/m^2/K] on bottom (slide 25)
...
pde_therm[[:boundary_left, :boundary_right]] = ThermalConvectionBC(h=h_edge, T_amb=T_amb)
...
weakform_visco = Viscoelasticity2D(rho_0=rho, mode=mode, model=LinearModel())
